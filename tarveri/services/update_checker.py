...
class UpdateCheckerService:
    ...
    async def check_for_updates(self) -> tuple[bool, int, str, str]:
        """
        Queries git to check if upstream branch is ahead of current local HEAD.
        Returns (is_update_available, behind_count, local_hash, remote_hash).
        """
        loop = asyncio.get_running_loop()

        def _git_check() -> tuple[bool, int, str, str]:
            try:
                ...
                upstream_branch = upstream_res.stdout.strip() or "origin/main"
                ...
                if not local_hash or not remote_hash or local_hash == remote_hash:
                    return False, 0, local_hash, remote_hash
                ...
                return behind_count > 0, behind_count, local_hash, remote_hash
            except Exception:
                return False, 0, "", ""

        is_avail, count, local_h, remote_h = await loop.run_in_executor(None, _git_check)

        if is_avail:
            msg = (
                f"A new TARVeri update is available on git upstream! "
                ...
                try:
                    ...
                    if hoster_user:
                        await hoster_user.send(
                            ...
                            f"A new update is available for your bot ({count} new commit(s)).\n\n"
        ...
        return is_avail, count, local_h, remote_h
